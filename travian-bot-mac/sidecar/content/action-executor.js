...
(function () {
  ...
  var TravianExecutor = {
    ...
    /**
     * Claim a claimable quest reward.
     * Travian shows quest rewards in several places:
     *   - The questmaster dialog (opened from the sidebar / questmaster icon)
     *   - Inline "Collect" buttons on finished quest rows
     *   - A reward notification popup
     *
     * Strategy: try multiple selectors in priority order. If questId is given,
     * try to find that specific quest's claim button first.
     *
     * @param {string|number} [questId] - Optional quest identifier for targeted claiming
     * @returns {Promise<boolean|object>}
     */
    claimQuest: async function (questId) {
      try {
        Logger.log('claimQuest: questId =', questId);
        await humanDelay(300, 600);

        // Priority 1: If a specific quest ID was given, find its claim button directly
        if (questId != null) {
          var specificBtn = trySelectors([
            '[data-questid="' + questId + '"] .claimReward',
            '[data-questid="' + questId + '"] .button.green',
            '[data-quest-id="' + questId + '"] button',
            '.quest_' + questId + ' .claimReward'
          ]);
          if (specificBtn) {
            await simulateHumanClick(specificBtn);
            Logger.log('claimQuest: clicked specific quest', questId);
            await humanDelay(600, 1200);
            // Dismiss any confirmation dialog
            var confirmOk = trySelectors(['.dialogButtonOk', '#ok', '.dialog button.green']);
            if (confirmOk) {
              await simulateHumanClick(confirmOk);
              Logger.log('claimQuest: confirmed dialog for quest', questId);
            }
            return true;
          }
        }

        // Priority 2: Generic claimable reward selectors (any finished quest)
        var claimSelectors = [
          // Questmaster dialog opened
          '.questlog .questRow.finished .claimReward',
          '.questlog .questRow.done .button.green',
          // Inline claim buttons in quest list
          '.questSlot.done .button',
          '.questItemSlot.rewardAvailable .button',
          // Reward popup / notification
          '.rewardCollect button',
          '.questReward .button.green',
          '.questDialog .claimReward',
          // Generic claim text on a green button within quest context
          '.quest .button.green'
        ];

        var btn = trySelectors(claimSelectors);
        if (!btn) {
          Logger.warn('claimQuest: no claimable quest button found');
          return { success: false, reason: 'button_not_found', message: 'No claimable quest reward found on current page' };
        }

        await simulateHumanClick(btn);
        Logger.log('claimQuest: clicked claim button');
        await humanDelay(600, 1200);

        // Confirm if a dialog appeared
        var confBtn = trySelectors(['.dialogButtonOk', '#ok', '.dialog button.green']);
        if (confBtn) {
          await simulateHumanClick(confBtn);
          Logger.log('claimQuest: confirmed reward dialog');
        }

        return true;
      } catch (e) {
        Logger.error('claimQuest error:', e);
        return false;
      }
    },
      ...
      switch (message.type) {
        ...
        case 'EXECUTE': {
          ...
          switch (action) {
            ...
            case 'claimQuest':
              actionResult = await TravianExecutor.claimQuest(params.questId);
              break;

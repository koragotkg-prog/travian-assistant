...
(function () {
  ...
  var TravianScanner = {
    ...
    // =========================================================================
    // Quest / Task Master (Task 17)
    // =========================================================================

    /**
     * Scan for available or claimable quests.
     * Returns { hasClaimable: bool, claimable: [{id, title}], hasNew: bool }
     *
     * Selectors are defensive — all wrapped in try/catch; returns empty list on failure.
     */
    scanQuests: function () {
      var result = { hasClaimable: false, claimable: [], hasNew: false };
      try {
        // Check for quest master notification dot / "finished quest" indicators
        var finishedItems = document.querySelectorAll(
          '.questSlot.done, .questItemSlot.rewardAvailable, [class*="quest"][class*="done"], ' +
          '.questMasterIcon .notificationCount, #questmaster .status-done'
        );

        // Also look for a top-bar notification badge on the quest button
        var questBtn = document.querySelector(
          '#questmaster, a[href*="quest"], .questMasterButton, .taskMaster'
        );
        if (questBtn) {
          var badge = questBtn.querySelector('.badge, .count, .notif');
          if (badge && parseInt(badge.textContent || '0') > 0) {
            result.hasNew = true;
          }
        }

        // Collect any claimable quests already visible in DOM (quest dialog open)
        var claimBtns = document.querySelectorAll(
          '.questDialog .claimReward, .questRow.finished .button, ' +
          '[id^="quest_reward_"], .taskListItem.done button'
        );

        claimBtns.forEach(function (btn, i) {
          var row = btn.closest('[data-questid], [id*="quest_"], .questRow, .taskListItem');
          var title = row
            ? (row.querySelector('.questTitle, .name, h4') || { textContent: 'Quest ' + i }).textContent.trim()
            : ('Quest ' + i);
          result.claimable.push({ id: row ? (row.dataset.questid || ('quest_' + i)) : ('quest_' + i), title: title });
        });

        if (finishedItems.length > 0 && result.claimable.length === 0) {
          result.hasClaimable = true; // questmaster has finished items but dialog not open
        }
